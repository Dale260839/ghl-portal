...
### D-016 · Dale is the project lead on Project Hub
**Decided:** 2026-08-21 (huddle with Chris) · **Owner:** Dale

Chris: *"I want you to own this. You're the project lead on this."* Ownership now
covers the GHL build as well as the Hub codebase — objects, custom fields,
pipeline, tags, triggers, the Custom Menu Link, and the connection wiring.

**The condition attached.** Chris asked for contact daily or every other day — a
ten-minute huddle or a short video, not a long meeting. His stated reason: without
it he builds an expectation in his head and then feels progress has gone stagnant.
Treat the check-in as part of the deliverable, not overhead.

**Access needed:** admin on the Alliance For Contractors sub-account, via Pat.

---

### D-017 · BuildSuite is the scope-of-work and proposal engine only
**Decided:** 2026-08-21 (huddle with Chris) · **Narrows D-009's framing**

Chris, verbatim: *"I want BuildSuite to only be the scope of work and proposal
engine."* Once a proposal is signed and the project is created, **GoHighLevel owns
operations** — schedule, tasks, day-to-day.

BuildSuite continues to own the commercial record: contract amount, original
estimate, markup, margin. GHL never re-owns those. The Hub reads the financial and
identity facts from BuildSuite and the operational state from GHL.

**Why recorded:** Chris pushed back on BuildSuite appearing to stay in the loop
after handoff, and asked directly whether the build was running "back through
BuildSuite instead of GoHighLevel." Answer: GHL for operations, BuildSuite for the
money. Keep BuildSuite's surface area minimal.

---

### D-018 · The join key is the APS job number, generated by BuildSuite
**Decided:** 2026-08-21 (huddle with Chris) · **Resolves D-010**

The shared key across BuildSuite, GHL and the Hub is the contractor job number —
`APS-081` form. Chris: *"we'll use APS as the main one."*

**Two conditions, both agreed in the call:**

1. **It lives in a dedicated field on both sides — never the job title.** A key
   held in a name breaks silently on one rename or typo, and a silent break means
   a job shows the wrong data to the wrong person.
2. **BuildSuite generates it automatically on project creation; GHL stores it.**
   One system owns the number, the other copies it. A human typing it on both
   sides produces mismatches.

**Open, to confirm with Sing:** can BuildSuite auto-assign this number and expose
it in a readable field? If not, fall back to `ghl_opportunity_id` as the real join
key and keep `APS-081` as a human-readable label only.

**Conflict to reconcile.** Artifact 25 (ProjectHub Setup SOP) names a *ProjectHub
Project ID* as the primary active-project tracking key, with the BuildSuite
Project ID as a reference ID — a third ID system. D-018 is what was agreed
verbally; the SOP has not been reconciled to it.

---

### D-019 · Payments route to the contractor's own Stripe, through GHL
**Decided:** 2026-08-21 (huddle with Chris) · **Resolves the in-app-invoicing question**

Chris: *"we'll hook the payments up to like the Stripe from the contractors'
accounts."* The Hub does not process cards and does not take on card-payment
compliance. Invoices and payment route through GHL's native portal against the
contractor's own Stripe connection.

The Hub's Payments screen shows invoice history, outstanding balance and what is
due, and links out. A visibility surface, not a billing system.

---

### D-020 · Field crews never touch GHL. The field view is deliberately thin.
**Decided:** 2026-08-21 (huddle with Chris)

Chris: *"I don't want the field people to log in to GoHighLevel... I want to be
able to actually have it like an app."* Confirmed as already built — the Field
Interface is a separate mobile-first surface in the Hub.

**Two corrections to what exists:**

1. **The field view should be thinner than it is.** Chris scoped it to assigned
   work, schedule, tasks, field updates, photos/videos and conversations — not the
   full picture the contractor sees.
2. **Subcontractors are a fourth role, narrower than field crew** — assigned scope
   only. Matches Artifact 25. Not built.

**New requirement:** a contractor assigns a task from the notes and the field user
is notified — Chris's *"they get a ding."* Not built.

---

### D-021 · Build from Chris's artifacts. Do not reinvent them.
**Decided:** 2026-08-21 (huddle with Chris)

Chris, on the friction between his documentation and this build: *"I only want one
source of truth... if Claude is our agent to build all these back end systems,
then it needs to have this kind of information — don't rewrite it, don't redo it.
Just let us know what are the points we need to enter in."*

**Standing rule for the GHL build.** Read the relevant artifacts first — Phase 5B
(GHL / CRM / Automation Deployment, artifacts 68–80), Phase 6 (BuildSuite /
ProjectHub Execution Layer, 82–93), Phase 7 (Pilot Launch, 94–101), and the
ProjectHub Setup SOP (Artifact 25). Where an artifact already names a field, a
stage, a tag or a custom value, **use that name.** Where something is missing or
contradicts `docs/ARCHITECTURE.md`, raise it as a gap — do not resolve it by
picking a side.

The custom values and tags Chris expects are already specified in his artifacts.
Creating custom objects in a live sub-account before reading them risks naming
that has to be redone after records exist.

---

### D-022 · Open questions Chris raised and did not resolve
**Recorded:** 2026-08-21 (huddle with Chris)

Not decisions — carried forward so they do not get lost.

| | Question | Chris's position |
|---|---|---|
| **Storage** | Where do client-uploaded photos, videos and documents live — GHL media storage, Supabase, or the contractor's own storage? | *"I don't know that yet."* Wants it settled with Sing. |
| **Alliance Protection / Warranty** | Still to be designed. Artifact 25 requires eligibility-safe language — setup alone never guarantees coverage. | To design. |
| **Timeline** | *"I don't want this to take two or three months to build out. If that's the case then we put it on hold right now, because we need to get clients on."* | Real constraint. Do not commit a date before seeing the sub-account. |
