...
import { raiseClientIssue } from '@/lib/actions/issues';
    ...
    <div className="space-y-6">
      ...
      {project.allowIssueSubmission && (
        <Card className="px-5 py-4">
          <h2 className="text-sm font-semibold text-navy-900">Raise an issue or request</h2>
          <p className="mt-0.5 text-xs text-navy-400">
            It goes straight to your contractor and appears below with its status.
          </p>
          <NoticeForm
            action={raiseClientIssue}
            className="mt-3 space-y-2.5"
            noticeClassName="rounded-lg border border-navy-100 bg-navy-50 px-3 py-2 text-sm text-navy-800"
          >
            <input type="hidden" name="projectId" value={project.buildsuiteProjectId} />
            <input
              name="issueTitle"
              required
              placeholder="In a few words, e.g. Tap in the main bathroom drips"
              className="w-full rounded-lg border border-navy-200 px-3.5 py-2.5 text-sm"
            />
            <textarea
              name="description"
              rows={3}
              placeholder="Anything that helps — where it is, when it started"
              className="w-full rounded-lg border border-navy-200 px-3.5 py-2.5 text-sm"
            />
            <SubmitButton className="rounded-lg bg-navy-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-navy-800">
              Send to my contractor
            </SubmitButton>
          </NoticeForm>
        </Card>
      )}
