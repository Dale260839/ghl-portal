...
export interface SignInRequestDeps {
   ...
   *
   * Was `contacts?: readonly Contact[]` — fixture rows matched in process. Main
   * replaced that on 2026-09-03: the match now runs inside the database and
   * `client_email` is never selected, so no client address reaches this
   * process. The reader is required rather than optional, because a default
   * would be a second place the gate could live.
   */
  readonly reader: ClientLoginReader;
  readonly now?: number;
  readonly jti?: string;
}
...
export async function requestSignInLink(
  ...
  const begun = await beginClientVerification(email, projectCode, deps.secret, deps.reader, {
    now: deps.now,
    jti: deps.jti,
  });
