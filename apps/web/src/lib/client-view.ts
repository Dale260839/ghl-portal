...
/**
 * The §9.1 gate and the §9.3 deny-list, applied server-side.
 *
 ...
 * The `server-only` import above makes that structural: if any of this is ever
 * imported into a client component, the build fails.
 *
 * Two layers on purpose:
 *   1. An explicit ALLOW-LIST — the client DTO is constructed field by field,
 *      so a new internal field added to `Project` cannot reach a client by
 *      default. Deny-lists fail open; allow-lists fail closed.
...
/**
 * Builds the client-safe projection of a project, or returns the reason it was
 * denied. Returning the reason rather than a bare null keeps denials debuggable
 * in logs without logging the record.
 */
export function toClientProject(
  project: Project,
  requester: Contact,
): { allowed: true; view: ClientProjectView } | { allowed: false; reason: GateDenialReason } {
